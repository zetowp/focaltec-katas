...
public class FileSystemCalculator {
    ...
    public void calculateRealFileSystem() {
        ...
        assertEquals(117l, new DirBuilder(new File("src/main/java/katas/fileSize/FileSystem.java")).getSize());
    ...
    public void counterFileSystem_files() throws IOException {
        FileSystemCounter counter = new FileSystemCounterFile();
        assertEquals(1, new DirBuilder(new File("src/main/java/katas/fileSize/FileSystem.java")).count(counter));
        long length = new DirBuilder(new File(".")).count(counter);
        File f1 = new File("toAdd");
        f1.createNewFile();
        assertEquals(length + 1, new DirBuilder(new File(".")).count(counter));
        f1.delete();
        assertEquals(length, new DirBuilder(new File(".")).count(counter));
    ...
    public void counterFileSystem_Chars() throws IOException {
        FileSystemCounter counter = new FileSystemCounterChar();
        assertEquals(117, new DirBuilder(new File("src/main/java/katas/fileSize/FileSystem.java")).count(counter));
        long length = new DirBuilder(new File(".")).count(counter);
        File f1 = new File("toAdd");
        f1.createNewFile();
        assertEquals(length, new DirBuilder(new File(".")).count(counter));
